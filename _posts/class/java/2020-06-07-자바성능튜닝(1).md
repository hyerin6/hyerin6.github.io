...
하나의 JSP나 스윙퍼럼 화면에 모든 처리 로직을 모아 두는 것이 아니라      
모델 역할, 뷰 역할, 컨트롤러 역할을 하는 클래스를 각각 만들어서 개발하는 모델이다.      


#### - J2EE 디자인 패턴이란?      

우선, 시스템을 만들기 위해서 전체 중 일부 의미 있는 클래스들을 묶은 각각의 집합을      
디자인 패턴이라고 생각하면 된다.   

J2EE 디자인 패턴에 대해 간단히 알아보자.   
먼저 Sun 에서 제공했던 Core J2EE 패턴 카탈로그를 보자. (책에 그림이 첨부되어 있음)  

몇가지 꼭 알아둬야 하는 패턴의 특징을 알아보자.   

- Business Delegate : 비즈니스 서비스 접근을 캡슐화하는 패턴이다.   
   
- Session Facade : 비즈니스 티어 컴포넌트를 캡슐화하고, 원격 클라이언트에서 접근할 수 있는 서비스를 제공하는 패턴이다.     
    
- Data Access Object : 일명 DAO 라고 많이 알려져있다. DB에 접근을 전담하는 클래스를 추상화하고 캡슐화한다.     

- Service Locator : 서비스와 컴포넌트 검색을 쉽게 하는 패턴이다.      

- Transfer Object : 일명 Value Object 패턴이라고 알려져있다. 데이터를 전송하기 위한 객체에 대한 패턴이다.        


이 중에서 성능과 관련이 있는 Transfer Object 와 Service Locator 패턴에 대해서 공부해보자.  
...
성능상으로 따져 볼 때 getter() 과 setter()를 만들지 않는 것이 더 빠르다.      
하지만, 정보를 은닉하고 모든 필드의 값들을 아무나 마음대로 수정할 수 없게 하려면       
getter/setter 메소드를 작성하는 것이 바람직하다.           


getter/setter 을 사용하면 getEmpName() 메서드처럼       
empName이 null값이라도 null을 리턴하지 않고 길이가 0인 String을 리턴한다.          


즉, Transfer Object를 잘 만들어 놓으면 각 소스에서 일일이 null 체크를 할 필요가 없어진다.    

여기서 한 가지 짚고 넘어가야 할 것은 toString() 메서드 부분이다.   
Transfer Object를 구현할 때 반드시 이 메서드를 구현하는 것이 좋다.    
그렇지 않고 toString() 을 사용하면 알 수 없는 값을 리턴한다.    



#### - Service Locator 패턴     

이 디자인 패턴은 정말 성능에 영향을 준다.    


```java  
private InitialContext ic;
private Map cache;
private static ServiceLocaror me;
...
// 이하 생략 책에 코드가 첨부되어 있음  
```  

Service Locator 패턴은 예전에 많이 사용했던 EJB의 EJB Home 객체나 DB의 DataSource를 찾을 때   
소요되는 응답 속도를 감소시키기 위해 사용된다.    

위 소스를 간단히 보면, cache라는 Map 객체에 home 객체를 찾은 결과를 보관하고 있다가 누군가 그 객체를 필요로 할 때   
메모리에서 찾아서 제공하도록 되어 있다.   

만약 해당 객체가 cache라느 Map에 없으면 메모리에서 찾는다.      



## story 3. 왜 자꾸 String을 쓰지 말라는 걸까 ?    
