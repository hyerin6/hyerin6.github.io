...
그러면 어떻게 성능을 높여야 할까 잠시 고민해봤는데 오히려 성능상 이슈와 Dead Lock에 대한 이슈로 Auto Increment를 강력하게 추천한다.           
그래서 bulk insert에 대한 개념은 알고있는게 좋고 지금 프로젝트에서 Auto Increment 설정을 그대로 유지하는 게 좋겠다는 결론이 났다.            
